...
JOB1=$(mini-slurm submit --cpus 2 --mem 2GB --priority 5 \
...
JOB2=$(mini-slurm submit --elastic --cpus 4 --min-cpus 2 --max-cpus 8 --mem 4GB --priority 5 \
...
JOB3=$(mini-slurm submit --cpus 4 --mem 4GB --priority 3 \
...
JOB4=$(mini-slurm submit --elastic --cpus 2 --min-cpus 1 --max-cpus 6 --mem 2GB --priority 4 \
...
for i in {1..5}; do
    ...
    mini-slurm queue --status RUNNING 2>/dev/null || echo "  No running jobs"
    
    # Show node assignments
    for JOB in $JOB1 $JOB2 $JOB3 $JOB4; do
        mini-slurm show $JOB 2>/dev/null | grep -E "(Status|CPUs|Nodes|Elastic)" || true
    done
done
...
for JOB in $JOB1 $JOB2 $JOB3 $JOB4; do
    echo
    echo "--- Job $JOB ---"
    mini-slurm show $JOB | grep -E "(Status|CPUs|Nodes|Elastic|Switches|distance|Wait|Runtime)" || true
done
