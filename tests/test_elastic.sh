...
python3 mini-slurm.py scheduler --total-cpus 8 --total-mem 16GB --elastic-threshold 50.0 > /tmp/scheduler.log 2>&1 &
SCHEDULER_PID=$!
...
JOB1=$(python3 mini-slurm.py submit --elastic --cpus 2 --min-cpus 2 --max-cpus 8 --mem 4GB --priority 5 \
...
JOB2=$(python3 mini-slurm.py submit --cpus 4 --mem 4GB --priority 10 \
...
for i in {1..60}; do
    RUNNING=$(python3 mini-slurm.py queue --status RUNNING 2>/dev/null | grep -c RUNNING || echo "0")
